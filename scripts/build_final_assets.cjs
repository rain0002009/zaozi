...
async function main() {
  ...
  // Clean hand at (172, 153), cuff at (164, 137)
  for (let y = 138; y <= 165; y++) {
  ...
  // Shoulder at (148, 112), cuff at Y=146.
  ...
  // Rounded shoulder cap around (148, 112) with radius 12
  ...
  // Robe torso from collar (101) to hem (162)
  ...
  // Calligraphic "人" legs from 163 down to 206
  for (let y = 163; y <= 206; y++) {
    for (let x = 75; x <= 175; x++) {
      // Exclude old knife (which was X < 100 for Y <= 186)
  ...
  // (1.5) Rear Arm Front: Mirror clean right arm across X=128
  // Shoulder at (108, 112), elbow at (92, 137), hand at (84, 153)
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
  ...
  // (2.3) Arms Back:
  // Front arm on viewer's left (shoulder 108, 112)
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      const srcX = 128 + (128 - x);
      if (srcX >= 0 && srcX < w) {
  ...
  // Rear arm on viewer's right (shoulder 148, 112)
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      const idx = (y * w + x) * 4;
  ...
  // (3.2) Side Body: Complete side torso from collar to hem, and legs down to 206
  // Back contour is at X >= 110, front contour is at X <= 144.
  // Old backward knife at X < 110 is completely excluded!
  for (let y = 101; y <= 206; y++) {
    for (let x = 110; x <= 144; x++) {
      const idx = (y * w + x) * 4;
      if (sd[idx + 3] > 0) {
  ...
  // Forearm emerges from (156, 135) to grip at (170, 148)
  for (let y = 134; y <= 160; y++) {
    for (let x = 152; x <= 178; x++) {
      const idx = (y * w + x) * 4;
      // Map front hand to side grip: front grip is (172, 153), side grip is (170, 148) => srcX = x + 2, srcY = y + 5
      const srcX = x + 2;
      const srcY = y + 5;
      if (srcX >= 0 && srcX < w && srcY >= 0 && srcY < h) {
        const srcIdx = (srcY * w + srcX) * 4;
        if (frontForearmFront[srcIdx + 3] > 0) {
          copyPixelAt(frontForearmFront, frontForearmSide, srcIdx, idx);
        }
      }
    }
  }
  ...
  console.log('Saved final ren_anchor_guide.png');
}
