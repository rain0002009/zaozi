...
import { gameState, ROUTES, RouteId } from '../state/GameState';
...
export class RouteScene extends Phaser.Scene {
  ...
  private drawMap(area: number): void {
    ...
    ink.lineBetween(170, 386, 410, 270);
    ink.lineBetween(170, 386, 410, 500);
    ink.lineBetween(614, 270, 848, 386);
    ink.lineBetween(614, 500, 848, 386);
    ...
    const routes: RouteId[] = area === 1 ? ['wilds', 'ruins'] : ['ember', 'rift'];
    this.drawRouteCard(routes[0], 380, 358, false);
    this.drawRouteCard(routes[1], 644, 358, false);
  }
  ...
  private drawRouteCard(routeId: RouteId, x: number, y: number, boss: boolean): void {
    ...
    const character = boss ? '墨' : routeId === 'wilds' ? '荒' : routeId === 'ruins' ? '碑' : routeId === 'ember' ? '火' : '鬼';
    ...
    const enter = this.add.text(x, y + 112, boss ? '迎 战' : '踏 入', { fontSize: '17px', color: '#1e241f', backgroundColor: '#d7c796', padding: { x: 20, y: 9 } }).setOrigin(0.5);
